...
function Add_Internship() {
    const api_endpoint = process.env.REACT_APP_SERVER_URL + '/api/internships/add';
    ...
    const handleSubmit = async (event) => 
        {
            ...
            internData.append('batch', formData.batch);
            internData.append('company_name', formData.company_name);
            internData.append('position', formData.position);
            internData.append('stipend', formData.stipend);
            internData.append('eligibility', formData.eligibility);
            internData.append('mode', formData.mode);
            internData.append('location', formData.location);
            internData.append('duration', formData.duration);
            internData.append('application_link', formData.application_link);
            internData.append('application_deadline', formData.application_deadline);
            if (formData.logo) 
            {
                internData.append('logo', formData.logo);  // Append the image file if selected
            }
            ...
            try 
            {
                ...
                if (response.ok) 
                {
                    ...
                    window.alert('New Internship Added successfully!');
                } 
                else 
                {
                    console.error('Failed to add new Internship');
                    window.alert('Failed to add new Internship');
                }
            } 
            catch (error) 
            {
    ...
    return (
        <div className='form-container'>
            <form onSubmit={handleSubmit}>
                <div className='form-title'>Add New Internship</div><br />
                ...
                <input className='input' type='text' id='stipend' name='stipend' value={formData.stipend} onChange={handleChange} required />
                <br />
                ...
                <input className='input' type='text' id='duration' name='duration' value={formData.duration} onChange={handleChange} required></input>
                <br />
