...
const Login = () => {
  ...
  const { universityRegId, password } = formData;
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      const data = {
        universityRegId: event.target.universityRegId.value,
        password: event.target.password.value,
      };
      const res = await axios.post(process.env.REACT_APP_SERVER_URL + '/api/students/login', formData);
  ...
  <div>
    ...
    <div className='form-container'>
      ...
      <form onSubmit={onSubmit}>
        ...
        <br/>
        <input className='input'type="text" name="universityRegId" value={universityRegId} onChange={onChange} required />
        <br/>
